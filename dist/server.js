...
export class AgentPushKitClient {
    ...
    async call(operationId, input) {
        ...
        const body = Object.fromEntries(inputs.body.filter((name) => input[name] !== undefined).map((name) => [name, input[name]]));
...
const MUTATING = { readOnlyHint: false, idempotentHint: false };
const IDEMPOTENT = { readOnlyHint: false, idempotentHint: true };
...
export function createAgentPushKitServer(client) {
    const server = new McpServer({ name: 'agent-push-kit', version: openApi.info.version }, { instructions: 'Complete Agent Push Kit access for the user represented by AGENT_PUSH_KIT_TOKEN.' });
    for (const tool of tools) {
        server.registerTool(tool.name, {
            description: tool.description,
            inputSchema: operationInputSchema(tool.operationId),
            annotations: tool.annotations,
        }, async (input) => result(await client.call(tool.operationId, input)));
    }
    return server;
}
